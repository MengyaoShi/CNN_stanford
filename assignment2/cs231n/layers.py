...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for n in range(0,N):
        for c in range(0,C):
            x_pad[n,c,:,:]=np.pad(x[n,c,:,:],(conv_param['pad'],conv_param['pad']),'constant',constant_values=(0,0))
...
def conv_backward_naive(dout, cache):
    ...
    for f in range(0, F):
        for n in range(0,N):
            for i in range(0,int((H+2*pad-HH)/stride+1)):
                for j in range(0,int((W+2*pad-WW)/stride+1)):
...
def max_pool_forward_naive(x, pool_param):
    ...
    for n in range(0,N):
        for c in range(0,C):
            for i in range(0, int((H-pool_height)/stride+1)):
                for j in range(0, int((W-pool_width)/stride+1)):
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(0,N):
        for c in range(0, C):
            for i in range(0,int((H-pool_height)/stride+1)):
                for j in range(0,int((W-pool_width)/stride+1)):
                    patch=x[n,c,(i*stride):(i*stride+pool_height), (j*stride):(j*stride+pool_width)]
                    maxPatch=np.max(patch)
                    idx=np.where(patch==maxPatch)
                    dx[n,c,i*stride+idx[0],j*stride+idx[1]] =dout[n,c,i,j]*1
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
